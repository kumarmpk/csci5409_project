...
app.get('/api/jobs/:jobName/', (req, res) => {
    var callback = (jobToFetch, error) => {
        if (error) {
            return res418.status(404).send('error occurred while fetching job from the database');
        }
        if (Object.keys(jobToFetch).length === 0) {
            return res.status(404).send('job not found, please enter another job name');
        }
...
app.get('/api/parts/:jobName/', (req, res) => {
    ...
    let sqlQuery = 'select * from Jobs j inner join Parts p  on j.PartId = p.PartId where j.JobName=?'
